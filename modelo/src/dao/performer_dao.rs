...
impl<'a> PerformerDao<'a> {
    ...
    pub fn agregar(&self, performer: &Performer) -> rusqlite::Result<()> {
    ...
    pub fn buscar_por_nombre(&self, nombre: &str) -> rusqlite::Result<Option<Performer>> {
        let mut stmt = self.conexion.prepare("SELECT id_performer, id_type, name FROM performers WHERE name = ?1")?;
